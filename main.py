...
option = st.sidebar.radio("Select an option:", ["Statistical Analysis and Visualization", "General Query about Data", "Custom Visualization"])
...
def generate_visualization_prompt(df, chart_type, library, x, y, title):
    """
    Create a detailed prompt for generating visualizations using pandas-ai.

    Args:
        df (pandas.DataFrame): The dataframe to visualize.
        chart_type (str): The type of chart to be generated (e.g., 'bar', 'line', 'scatter').
    ...
    Returns:
        str: A detailed prompt to guide pandas-ai in generating the visualization.
    """
    data_sample = df.head().to_string()
    data_summary = df.describe().to_string()

    prompt = f"""
    You are a data visualization assistant. Create a {chart_type} chart using the {library} library.
...
def generate_visualization_using_pandasai(data, prompt):
    """
    Generate a visualization using pandas-ai based on the provided prompt.

    Args:
        data (pandas.DataFrame): The dataframe to visualize.
        prompt (str): The prompt specifying the visualization requirements.

    Returns:
        Visualization output based on the prompt.
    """
...
#os.environ['REPLICATE_API_TOKEN'] = "r8_b9UXgQgcaVhvOodIeOrs1qftIMmMEkl3jDojS"
...
if uploaded_file:
    ...
    if option == "Statistical Analysis and Visualization":
        ...
        # User inputs for visualization details
        st.header("Generate Visualization Using PandasAI")
        chart_type = st.selectbox("Select chart type:", ["bar", "line", "scatter"])
        library = st.selectbox("Select library:", ["matplotlib", "seaborn", "plotly"])
        x_column = st.selectbox("Select X-axis column:", df.columns)
        y_column = st.selectbox("Select Y-axis column:", df.columns)
        title = st.text_input("Enter the chart title:")
        ...
        if st.button("Generate Visualization"):
            with st.spinner("Generating visualization..."):
                visualization_output = generate_visualization_using_pandasai(df, prompt)
                st.write(visualization_output)
